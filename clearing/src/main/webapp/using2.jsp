...
<form action="usingController" method="post">
	<div class="center-pos">
		...
		<input type="date" id="Date" name = "date" value="<%=seldate %>" style="width:180px;height:38px;text-align: center;">
	...
	<script type="text/javascript">
		$('#Date').on('blur',function(){
			...
			if(now_date!==selDate){
				window.location.href="http://localhost:8081/clearing2/DateController?date="+selDate;
				
			}else{
				window.location.href="http://localhost:8081/clearing2/bookLaundryController";
			}
		});
		...
		$("select[name=time]").change(function(){
		 	...
		 	const selDate= document.querySelector("#Date").value;
		 	window.location.href="http://localhost:8081/clearing2/TimeController?time="+selTime+"&date="+selDate;//value값 가져오기
		});
	</script>
	<!-- js 달력 선택 제한 스크립트 -->
	<script>
